      ...
      <div className="grid h-full gap-6 lg:grid-cols-2">
        <div>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href="/plant" className="rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 cursor-pointer">Analytic</Link>
          </div>
        ...
        <div className="relative min-h-[300px] rounded-[32px] bg-gradient-to-br from-emerald-300 via-emerald-100 to-white">
          ...
          <Link href="/plant" className="absolute left-6 top-6 flex items-center gap-2 rounded-full bg-white/80 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-white cursor-pointer">
          ...
          <div className="absolute bottom-6 left-1/2 flex -translate-x-1/2 gap-2">
            {[1, 2, 3, 4].map((dot) => (
              <button
                ...
                className={`h-2 w-2 rounded-full transition cursor-pointer ${dot === activeSection ? "bg-slate-900" : "bg-white/60 hover:bg-white/80"}`}
              />
            ))}
          </div>
        </div>
